...
console.log(block.footnotes.gfm.paragraph)
/**
 * Block Lexer
 */
...
InlineLexer.prototype.output = function(src) {
  ...
  // highlight
    ...
    if (this.options.mathjax) {
      if (cap = this.rules.math.exec(src)) {
          src = src.substring(cap[0].length);
          out += this.renderer.math(cap[2], cap[1]);
          continue;
      }
    }
...
Renderer.prototype.oembed = function(caption, href, props) {
...
Parser.prototype.tok = function() {
  switch (this.token.type) {
    ...
    case 'math': {
      return this.renderer.math(this.token.text, this.token.block);
    }
  }
};
